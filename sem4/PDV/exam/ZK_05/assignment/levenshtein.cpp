...
long distances_par(const std::vector<std::string> &vector, unsigned int &maxIdx) {
    long result = 0;
    std::atomic<unsigned int> maxDistance;
    maxDistance.store(0);
    std::atomic<unsigned int> maxIndex;
    maxIndex.store(0);
    ...
    for (int i=0; i<vector.size(); ++i)
        for (int j=i+1; j<vector.size(); ++j) {
            ...
            // Dulezite: pokud je maximalni vzdalenost nabyvana na nekolika indexech,
            // pamatujeme si vzdy jen ten nejmensi
            auto curMaxDistance = maxDistance.load();
            while (distance > curMaxDistance) {
                if (maxDistance.compare_exchange_strong(curMaxDistance, distance)) {
                    maxIndex.store(i * vector.size() + j);
