...
bool create_aux_grid(int rows, int cols, aux_grid_point*** aux_grid, int** grid, int num_of_sensors) {
    *aux_grid = (aux_grid_point**)malloc(sizeof(aux_grid_point*) * rows);
    if (*aux_grid == NULL) {
        fprintf(stderr, "Error allocating memory for aux_grid rows\n");
        return false;
    }
    ...
    for (int row = 0; row < rows; row++) {
        ...
        if ((*aux_grid)[row] == NULL) {
            fprintf(stderr, "Error allocating memory for aux_grid columns at row %d\n", row);
        ...
        for (int col = 0; col < cols; col++) {
            ...
            if ((*aux_grid)[row][col].num_of_points == NULL || (*aux_grid)[row][col].sum_of_elevations == NULL) {
                ...
                fprintf(stderr, "Error allocating memory for aux_grid points at row %d, col %d\n", row, col);
                return false;
            }
