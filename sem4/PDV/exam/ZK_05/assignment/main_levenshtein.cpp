...
int main(int argc, char* argv[]) {
    ...
    for(int i = 0 ; i < 5 ; i++) {
        ...
        long distance_par = distances_par(cVector, maxIndex);
        auto t_end_par = std::chrono::high_resolution_clock::now();
        double time_par = duration_cast<microseconds>(t_end_par - t_start_par).count() / 1000.0;

        std::cout << "Solution of "<< i+1  <<". instance generated in " << time_par << "ms" << std::endl
                  << "   Distance:     " << distance_par << std::endl
                  << "   Max distance idx:     " << maxIndex << std::endl;
    }
