...
int main(int argc, char* argv[]) {
    ...
    for(int i = 0 ; i < 10 ; i++) {
        ...
        std::cout << i + 1 << ". SEQUENTIAL: Checksum " << check_sum << " computed in " << duration << "ms" << std::endl;

        auto t_start_par = std::chrono::high_resolution_clock::now();
        filtering_par(copy_table, MAX_ITERATIONS);
        auto t_end_par = std::chrono::high_resolution_clock::now();
        double duration_par = duration_cast<microseconds>(t_end_par - t_start_par).count() / 1000.0;
        long long check_sum_par = generate_checksum(copy_table.getData());

        std::cout << i + 1 << ". PARALLEL: Checksum " << check_sum_par << " computed in " << duration_par << "ms" << std::endl;

        if (check_sum != check_sum_par) {
