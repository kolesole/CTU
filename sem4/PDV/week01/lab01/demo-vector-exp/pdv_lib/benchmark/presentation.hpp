...
namespace pdv {
    ...
    namespace _ {
        ...
        inline void print_benchmark_result(const benchmark_result_with_value<RetVal>& result) {
            ...
            if constexpr (std::is_floating_point_v<RetVal>) {
                _::StreamFlags flags(std::cout);
                // print floats with high precision, the default formatting drops quite a lot of decimal digits
                std::cout << std::setw(15) << "result: " << std::setprecision(15) << result.return_value;
            } else if constexpr (std::is_same_v<RetVal, bool>) {
                std::cout << std::setw(15) << "result: " << (result.return_value ? "true" : "false");
            } else if constexpr (_::is_ostreamable_v<RetVal>) {
        ...
        inline void print_benchmark_result(const benchmark_result_with_value<std::vector<T>>& result) {
            print_benchmark_result(static_cast<const benchmark_result&>(result));
            auto& vec = result.return_value;
            std::cout << std::setw(15) << "checksum: " << std::accumulate(vec.begin(), vec.end(), T{});
        }
