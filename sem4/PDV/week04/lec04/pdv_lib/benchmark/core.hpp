...
namespace pdv {
    ...
    inline T launder_value(const T& val) {
        ...
        static_assert(std::is_fundamental_v<T>, "Refusing to copy non-fundamental type m_value in pdv::launder_value");
    ...
    benchmark_raw(size_t iteration_count, BenchmarkFn fn) {
        ...
        for (size_t i = 0; i < iteration_count - 1; i++) {
            // ignore return values for the first `n - 1` iterations
            iter_durations.push_back(_::run_iteration(fn));
        }
