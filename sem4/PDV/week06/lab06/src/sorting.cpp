...
// Evaluacni funkce:
solution_t eval(std::string name, std::vector<elem_t>& unsorted, void (* sorter)(std::vector<elem_t>&),
                solution_pair_t* reference) {
    ...
    try {
        ...
        if (reference != nullptr && reference->first != copy) {
            ...
            if (reference != nullptr) speedup = (double) reference->second / (double) elapsed;
    ...
    return std::make_unique<solution_pair_t>(std::move(copy), elapsed);
}

int main() {
    // Nejprve si nagenerujeme pole velikosti N_ELEMS a ulozime do nej nahodne prvky z rozsahu [MIN_VALUE .. MAX_VALUE].
    auto unsorted = pdv::generate_random_vector<elem_t>(N_ELEMS, MIN_VALUE, MAX_VALUE);

    // Nyni muzeme testovat Vase implementace. Nejprve si napocteme referencni
    // reseni pomoci knihovni funkce std::sort.
    solution_t reference = eval("std::sort(...)           ", unsorted, [](std::vector<elem_t>& to_sort) {
